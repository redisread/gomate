...
const CACHE_KEY_PREFIX = "local-circle:v2:public:";
...
interface CacheEntry {
  data: PublicLocalCircle;
  storedAt: number;
}

const topLocationSchema = z.object({
  locationId: z.string(),
...
const cacheEntrySchema = z.object({
  data: z.object({
    ...
    topLocations: z.array(topLocationSchema),
  }).strict(),
  storedAt: z.number(),
}).strict();
...
function cacheKey(regionId: string, language: LocalCircleLanguage) {
  return `${CACHE_KEY_PREFIX}${regionId}:${language}`;
}
...
async function readPublicCache(
  ...
  try {
    ...
    if (age < 0 || age > CACHE_TTL_SECONDS * 1_000) return null;
    return parsed.data.data;
  } catch (error) {
    logger.warn("local_circle_cache_read_failed", safeErrorMetadata(error));
    return null;
  }
}
...
async function writePublicCache(
  ...
  if (!kv) return;
  try {
    const entry: CacheEntry = { data, storedAt: now };
    await kv.put(key, JSON.stringify(entry), {
      expirationTtl: CACHE_TTL_SECONDS,
    });
  } catch (error) {
    logger.warn("local_circle_cache_write_failed", safeErrorMetadata(error));
  }
}
...
export async function getLocalCircleHome(
  ...
  const key = cacheKey(params.regionId, params.language);
  let publicData = await readPublicCache(params.kv, key, now);
  if (!publicData) {
    publicData = await computePublicLocalCircle({ ...params, now });
    await writePublicCache(params.kv, key, publicData, now);
  }
