...
export function LocationFormSettingsFields({ formData, allTags, updateField }: LocationFormSettingsFieldsProps) {
  ...
  // P0-B T4：装备列表 drag&drop（gearEssential / gearOptional）— 与 tips/warnings 相同交互
  const handleGearDrop = React.useCallback((e: React.DragEvent, dropIdx: number, field: "gearEssential" | "gearOptional") => {
    e.preventDefault();
    const fromIdx = parseInt(e.dataTransfer.getData("text/plain"), 10);
    if (fromIdx === dropIdx) return;
    const next = moveItem(formData[field], fromIdx, dropIdx);
    updateField(field, next);
  }, [formData, moveItem, updateField]);
  return (<>
    <SectionCard icon={<Settings className="h-4 w-4" />} title={t("admin.formSettingsTitleRecommended")} collapsible defaultOpen={true}
      ...
      <SubSectionCard title={t("admin.formDecisionTitle")} defaultOpen={false}>
        ...
        <div className="space-y-4">
          ...
          {/* 停车说明（parkingAvailable !== false 才展示，false 时无需说明） */}
          {formData.parkingAvailable !== false && (
            <Field label={t("admin.formParkingInfoLabel")} hint={t("admin.formParkingInfoHint")}
              error={formData.parkingInfo.length > 100 ? t("admin.formParkingInfoHint") : undefined}>
              <input type="text" value={formData.parkingInfo} maxLength={100}
                onChange={(e) => updateField("parkingInfo", e.target.value)}
                placeholder={t("admin.formParkingInfoPlaceholder")}
                className={cn("w-full px-3 py-2 rounded-xl text-sm outline-none transition-all duration-150 border bg-white dark:bg-stone-800 text-stone-900 dark:text-stone-100 placeholder:text-stone-400 dark:placeholder:text-stone-500 border-stone-200 dark:border-stone-700 focus:ring-2 focus:ring-amber-200 focus:border-amber-400")} />
            </Field>
          )}

          {/* 必带装备 gearEssential */}
          <Field label={t("admin.formGearEssentialLabel")} hint={t("admin.formGearEssentialHint")}>
            <div className="space-y-2">
              {formData.gearEssential.map((item, idx) => (
                <div key={idx} draggable
                  onDragStart={(e) => handleDragStart(e, idx)}
                  onDragOver={handleDragOver}
                  onDrop={(e) => handleGearDrop(e, idx, "gearEssential")}
                  className="flex items-center gap-1.5 rounded-lg bg-stone-50/50 dark:bg-stone-800/50 hover:bg-stone-100 dark:hover:bg-stone-700/50 transition-colors cursor-grab active:cursor-grabbing p-1">
                  <GripVertical className="h-3.5 w-3.5 text-stone-300 shrink-0" />
                  <input type="text" data-gear-essential-input value={item} maxLength={20}
                    onChange={(e) => {
                      const next = [...formData.gearEssential]; next[idx] = e.target.value;
                      updateField("gearEssential", next);
                    }}
                    onBlur={() => {
                      if (item.trim() === "") {
                        updateField("gearEssential", formData.gearEssential.filter((_, i) => i !== idx));
                      }
                    }}
              ...
              {formData.gearEssential.length < 10 ? (
                <AddButton label={t("admin.formGearAdd")} onAdd={() => {
                  updateField("gearEssential", [...formData.gearEssential, ""]);
                  setTimeout(() => {
                    const inputs = document.querySelectorAll<HTMLInputElement>('[data-gear-essential-input]');
                    const last = inputs[inputs.length - 1];
                    last?.focus();
                  }, 0);
                }} />
              ) : (
                <p className="text-xs text-red-500">{t("admin.formGearMaxError")}</p>
              )}
            </div>
          </Field>
          ...
          <Field label={t("admin.formGearOptionalLabel")} hint={t("admin.formGearOptionalHint")}>
            <div className="space-y-2">
              {formData.gearOptional.map((item, idx) => (
                <div key={idx} draggable
                  onDragStart={(e) => handleDragStart(e, idx)}
                  onDragOver={handleDragOver}
                  onDrop={(e) => handleGearDrop(e, idx, "gearOptional")}
                  className="flex items-center gap-1.5 rounded-lg bg-stone-50/50 dark:bg-stone-800/50 hover:bg-stone-100 dark:hover:bg-stone-700/50 transition-colors cursor-grab active:cursor-grabbing p-1">
                  <GripVertical className="h-3.5 w-3.5 text-stone-300 shrink-0" />
                  <input type="text" data-gear-optional-input value={item} maxLength={20}
                    onChange={(e) => {
                      const next = [...formData.gearOptional]; next[idx] = e.target.value;
                      updateField("gearOptional", next);
                    }}
                    onBlur={() => {
                      if (item.trim() === "") {
                        updateField("gearOptional", formData.gearOptional.filter((_, i) => i !== idx));
                      }
                    }}
              ...
              {formData.gearOptional.length < 10 ? (
                <AddButton label={t("admin.formGearAdd")} onAdd={() => {
                  updateField("gearOptional", [...formData.gearOptional, ""]);
                  setTimeout(() => {
                    const inputs = document.querySelectorAll<HTMLInputElement>('[data-gear-optional-input]');
                    const last = inputs[inputs.length - 1];
                    last?.focus();
                  }, 0);
                }} />
