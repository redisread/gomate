...
/**
 * V2 Region 选择下拉组件。
 * 当前只消费后端返回的 service-enabled、city-level Region 实体。
 */
export function RegionSelect({
  ...
  const selectedRegion = useMemo(
    () => regions.find((region) => region.id === value),
    [regions, value]
  );

  const hotRegions = useMemo(
    () => regions.filter((region) => region.isHot).slice(0, HOT_REGION_MAX),
    [regions]
  );
  ...
  const filteredRegions = useMemo(() => {
    ...
    return regions
      .filter(
        (region) =>
          region.name.toLowerCase().includes(kw) ||
          region.nameEn?.toLowerCase().includes(kw) ||
          region.slug.toLowerCase().includes(kw) ||
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case "Enter":
          e.preventDefault();
          if (highlightIndex >= 0 && filteredRegions[highlightIndex]) {
            handleSelect(filteredRegions[highlightIndex].id);
          }
          break;
      }
    },
    [open, filteredRegions, highlightIndex, handleSelect]
  );
    ...
    >
      ...
      >
        <span
          className={cn(
            "truncate",
            selectedRegion ? "text-zinc-900" : "text-zinc-400"
          )}
          title={selectedRegion?.name}
        >
          {selectedRegion ? selectedRegion.name : t("common.selectRegion")}
        </span>
        <ChevronDown
          className={cn(
      ...
      {clearable && selectedRegion && !disabled && (
        <button
          type="button"
          aria-label={t("common.clear")}
          onClick={(e) => {
            e.stopPropagation();
            onChange(""); // 空串 → 表单层 PATCH 归一为 null（清空地区）
          }}
      ...
      {open && (
        ...
        >
          ...
          {/* 热门地区（仅无搜索词时显示） */}
          {!query.trim() && hotRegions.length > 0 && (
            <div className="px-3 pt-2 pb-1">
              <p className="mb-1.5 text-xs font-medium text-zinc-400">{t("common.hotRegions")}</p>
