...
import { Mountain, Heart, Mail, ArrowUp, MapPin, Users, Plus, ChevronDown } from "lucide-react";
import type { TranslationKey } from "@/i18n";
import { useI18n } from "@/hooks/useI18n";

/**
 * 页脚组件 - 响应式设计
 * PC/平板：四列网格布局（品牌卡片 + 三列链接）
 * 移动端：品牌区居中 + 手风琴折叠 + 社交图标底部独立行
 */
...
function MobileAccordionSection({ title, children }: MobileAccordionSectionProps) {
      ...
      <div className="relative max-w-6xl mx-auto px-6 sm:px-8">
            ...
            <div className="col-span-2 lg:col-span-1">
              <div className="card-base p-6 h-full flex flex-col">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="font-bold text-lg tracking-tight text-foreground">GoMate</span>
                </div>
            ...
            <div>
              ...
              <ul className="space-y-4 sm:space-y-3">
                {exploreLinks.map(({ href, label, icon: Icon }) => (
                  <li key={href}>
                    <a href={href} className="footer-link inline-flex items-center gap-2 text-sm text-muted-foreground group">
            ...
            <div>
              ...
              <ul className="space-y-4 sm:space-y-3">
                {supportLinks.map(({ href, label }) => (
                  <li key={href}>
                    <a href={href} className="footer-link text-sm text-muted-foreground">{label}</a>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="md:hidden py-8">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-sm text-muted-foreground mb-4">{t("common.tagline")}</p>
            <a
              href={`mailto:${t("common.contactEmail")}`}
          ...
          <div className="border-t border-border">
            {/* 探索 */}
            <MobileAccordionSection title={t("common.explore")}>
            ...
            {/* 支持 */}
            <MobileAccordionSection title={t("common.support")}>
              <ul className="space-y-3">
                {supportLinks.map(({ href, label }) => (
                  <li key={href}>
                    <a href={href} className="footer-link text-sm text-muted-foreground">{label}</a>
            ...
            {/* 法律 */}
            <MobileAccordionSection title={t("common.legal")}>
              <ul className="space-y-3">
                {legalLinks.map(({ href, label }) => (
                  <li key={href}>
                    <a href={href} className="footer-link text-sm text-muted-foreground">{label}</a>
          ...
          {/* 社交图标行 - 独立居中展示 */}
          <div className="flex items-center justify-center gap-3 py-6 border-b border-border">
            {/* 微信 */}
            <button
              onClick={() => setShowWechatQR(true)}
        ...
        {/* ── 版权栏（共享） ── */}
        <div className="border-t border-border py-8 sm:py-6 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <button
            onClick={scrollToTop}
            className="md:hidden inline-flex items-center gap-1.5 text-xs font-medium text-muted-foreground transition-all duration-200 hover:text-primary"
          >
            <ArrowUp className="h-3.5 w-3.5" />
            {t("common.backToTop")}
          </button>
