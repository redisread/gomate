...
- 访客看到 Hero、公开 Team、地点推荐与注册入口；首页不展示额外的三步产品介绍区域。
- 「本周正在集结」是滚动未来 7 天：SSR 与客户端共用当前时刻至 7×24 小时后的
  `startDateFrom` / `startDateTo` ISO 时间范围，只查询仍开放招募且未出发的 pending 队伍。
  时间条件在 API 分页前应用；客户端按用户地区刷新时重新计算范围。
- 登录用户看到下一次行程、推荐和 onboarding。
- 本地圈公共部分直接从 D1 读取并按请求合并当前用户数据，不把用户相关结果写入共享缓存。
- 全国地图读取 `/api/locations/stats` 的 Region DTO；地图统计和点位只包含同时具有纬度、经度的已发布地点。
