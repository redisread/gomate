...
**目标。** 管理员在完整地点表单中能明确区分“保存内容”和“改变发布状态”，不会因草稿或归档
地点进入公开路由而看到“地点不存在”。现有 `locations.status` 继续作为唯一发布状态来源，不新增
表、字段或发布接口。
...
- 后台完整表单用只读状态标识和状态相关操作代替通用状态下拉框。发布操作显式提交
...
**技术栈与结构。** 使用现有 Astro 7、React 18、TypeScript、Hono 和 Vitest，不增加依赖。
表单状态与写入编排位于 `src/components/features/location-form/use-location-form.ts`，操作区位于同目录
组件，页面组装位于 `src/components/features/location-edit-client.tsx`，文案进入现有三种 locale 的
`admin` namespace；API 与 `locations.status` 合同保持不变。

**代码风格。** 状态转换由调用方显式表达，不从按钮文案或 DOM 推断：

```ts
await handleSave("publish");
await handleSave("restore");
await handleSave("keep");
```

公开链接同样从已保存的服务端状态派生，不以待提交表单值为依据。

**命令。** 开发使用 `pnpm dev:wt`；实现期间运行聚焦 Vitest；交付前依次运行
`pnpm i18n:build`、`pnpm i18n:validate`、`pnpm lint`、`pnpm type-check`、`pnpm test`、
`pnpm test:server` 和 `pnpm build`。

**测试策略。** Vitest 覆盖 draft/published/archived 的操作可见性、返回链接、发布参数、发布前
校验、编辑保存后不跳转，以及首次创建只进入后台编辑路由；现有服务端测试继续证明只有完整地点
可以切换为 `published`。浏览器验证覆盖键盘焦点、移动端底部操作区和已发布地点的公开链接。

**边界。** 必须复用当前 Location DTO、统一 API error、i18n 和 `locations.status` 约束；数据库
schema、API 路径、依赖或归档语义如需变化必须另行确认。不得让草稿/归档地点进入公开详情、绕过
发布必填校验、直接修改生产 D1，或删除/弱化现有失败测试。

**完成条件。** 上述三种状态的操作矩阵、导航、成功/失败反馈和键盘行为均有自动化测试；编辑
草稿后返回管理列表不会出现公开 404；管理员可通过明确的“发布地点”操作完成发布；所有统一
Worker 门禁通过。

**开放问题。** 无。若本规格获批，后续计划和实现不得扩大到批量发布、定时发布、审批流或
`published_at`。
