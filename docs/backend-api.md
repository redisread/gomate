...
首页使用列表接口的 `startDateFrom` / `startDateTo` ISO 时间戳筛选滚动未来 7 天，
上下界均包含，结合 `lifecycle=pending` 排除已经出发的队伍；筛选先于 limit/cursor 分页。
Worker 每小时自动关闭结束满 24 小时、仍开放招募、未成团且未取消的队伍；已完成或
已关闭的队伍不变。自动关闭不删除详情、成员或申请，也不修改现有报名截止规则。
