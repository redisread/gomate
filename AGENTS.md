...
本文件为 Codex (Codex.ai/code) 提供操作本代码仓库的指引。
...
**症状**：页面无法访问、客户端组件无法加载、热更新失效

**原因**：多个开发服务器进程同时运行导致端口冲突
...
# 2. 一键清理并重启
kill -9 $(lsof -ti:5432) $(lsof -ti:8799) 2>/dev/null; pnpm dev
```

**避免方法**：
- 始终使用 `Ctrl+C` 正确终止开发服务器
- 避免直接关闭终端窗口（进程可能继续运行）
- 启动前检查端口是否被占用
...
- 所有代码必须使用 UTF-8 编码，中文字符完整保留，不得出现编码错误。
