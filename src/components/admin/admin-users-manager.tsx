...
export function AdminUsersManager({ adminId }: { adminId: string }) {
  ...
  const load = React.useCallback(
    async (search = "", cursor?: string, append = false) => {
      ...
      try {
        ...
        setUsers((current) =>
          append ? [...current, ...loadedUsers] : loadedUsers,
        );
  ...
  return (
    <div className="space-y-6">
      <form
        ...
        role="search"
        className="flex gap-3 rounded-xl bg-card p-4 shadow-sm sm:p-5"
      >
        ...
        <button className="min-h-11 self-end rounded-lg bg-primary px-4 font-semibold text-primary-foreground">
        ...
        <div className="space-y-3">
            ...
            return (
              ...
              >
                ...
                <button
                  ...
                  disabled={isCurrent}
                  aria-label={
                    isCurrent ? t("admin.management.currentUser") : undefined
                  }
                  onClick={() => void changeRole(user)}
                  className="min-h-11 rounded-lg border border-border px-4 text-sm font-semibold disabled:cursor-not-allowed disabled:opacity-50"
                >
                  {isCurrent
                    ? t("admin.management.currentUser")
                    : t(
                        user.role === "admin"
          ...
          {nextCursor && (
            <button
              ...
              onClick={() => void load(activeQuery, nextCursor, true)}
