...
export function AdminQuickLocationForm({
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const result = await adminJsonOrThrow<{
        success: true;
        location: Location;
      }>(createResponse, t, "admin.management.saveFailed");
      setCreated(result.location);
      if (tagIds.length > 0) {
        try {
          const tagsResponse = await fetchAPI(
            `/locations/${result.location.id}/tags`,
            {
              method: "PUT",
    ...
    return (
      <div role="status" className="space-y-4">
        ...
        {optionalSaveFailed && (
          <p
            role="alert"
            className="rounded-lg bg-amber-500/10 p-3 text-sm text-foreground"
          >
            {t("admin.quickDraft.optionalSaveFailed")}
          </p>
        )}
        <a
          href={`/admin/locations/${created.id}/edit`}
          className="inline-flex min-h-11 w-full items-center justify-center rounded-lg bg-primary px-4 text-sm font-semibold text-primary-foreground"
        >
    ...
    <form onSubmit={submit} className="space-y-4">
      ...
      <details className="rounded-lg border border-border p-3">
        <summary className="min-h-11 cursor-pointer py-2 text-sm font-semibold">
          {t("admin.quickDraft.optionalFields")}
        </summary>
        <div className="mt-3 space-y-4">
          ...
          <fieldset className="space-y-2">
            <legend className="text-sm font-medium">
              {t("admin.quickDraft.activityTypes")}
            </legend>
            <div className="flex flex-wrap gap-2">
              {ACTIVITY_TYPES.map((activityType) => (
                ...
                >
                  <input
                    type="checkbox"
                    checked={activityTypeIds.includes(activityType)}
                    onChange={() =>
                      toggle(activityType, activityTypeIds, setActivityTypeIds)
                    }
                  />
      ...
      <button
        disabled={saving}
        className="min-h-11 w-full rounded-lg bg-primary px-4 text-sm font-semibold text-primary-foreground disabled:opacity-50"
      >
