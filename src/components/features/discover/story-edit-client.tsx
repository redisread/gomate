...
function StoryEditClientInner({ storyId }: StoryEditClientProps) {
  ...
  const validateRequiredOnBlur = (field: "title" | "content") => {
    ...
    const optionalTeamTitle = field === "title" && Boolean(form.teamId);
    setFieldErrors((prev) => ({
      ...prev,
      [field]:
        optionalTeamTitle || form[field].trim() ? undefined : t(errorKey),
    }));
  };
  ...
  const isRequiredEmpty =
    !form.content.trim() || (!form.teamId && !form.title.trim());
  ...
  React.useEffect(() => {
    if (uploadMessage) {
      showToast({ type: "error", message: t(uploadMessage) });
    }
  }, [uploadMessage, showToast, t]);
      ...
      <div className="sticky top-0 z-40 border-b border-border bg-card backdrop-blur-sm">
        <div className="mx-auto flex h-14 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-1 items-center justify-end gap-3 sm:flex-none">
            <button
              onClick={handleSave}
              disabled={isSaving || isRequiredEmpty}
            ...
            >
              ...
              {t("common.save")}
              {isDirty && (
                <span className="sr-only">{t("common.unsavedChanges")}</span>
              )}
            </button>
          </div>
        </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
        ...
        {draftAvailable && (
          <div className="mb-6 flex items-center justify-between gap-3 rounded-md border border-border bg-accent px-4 py-3">
            <p className="text-sm text-accent-foreground">
              {t("content.discover.edit.draftBanner")}
            </p>
            <div className="flex gap-2">
              ...
              >
                {t("content.discover.edit.restoreDraft")}
              </button>
              <button
                onClick={handleDiscardDraft}
                className="btn-ghost px-3 py-1 text-xs"
              >
                {t("content.discover.edit.discardDraft")}
              </button>
            </div>
          </div>
        )}
          ...
          <div className="mb-6 flex items-center justify-between gap-3 rounded-md border border-border bg-accent px-4 py-3">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="btn-primary px-3 py-1 text-xs"
              >
                {t("content.discover.edit.saveRetry")}
              </button>
              <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            >
              {basicExpanded
                ? t("content.discover.edit.collapseMore")
                : t("content.discover.edit.expandMore")}
              {basicExpanded ? (
            ...
            >
              <FormField
                label={t("content.discover.edit.statusLabel")}
                htmlFor="story-edit-status"
              >
                <Select
                  ...
                  options={[
                    {
                      value: "published",
                      label: t("content.discover.edit.statusPublished"),
                    },
                    {
                      value: "draft",
                      label: t("content.discover.edit.statusDraft"),
                    },
                  ]}
                />
              </FormField>
              <FormField
                label={t("content.discover.edit.imagesLabel")}
                hint={t("content.discover.edit.imagesKeepHint")}
              >
                {form.images.length > 0 ? (
                  <div className="grid grid-cols-2 gap-3">
                    {form.images.map((image, index) => (
                      ...
                      >
                        <img
                          src={image}
                          alt={t("content.discover.edit.imageAlt", {
                            index: index + 1,
                          })}
                        ...
                        <button
                          ...
                          aria-label={t("content.discover.edit.removeImage", {
                            index: index + 1,
                          })}
                  ...
                  <p className="rounded-lg border border-dashed border-border px-3 py-6 text-center text-sm text-muted-foreground">
                    {t("content.discover.edit.noImages")}
                  </p>
                )}
              </FormField>
              <FormField
              ...
              >
                {form.locationName ? (
                  <div className="flex items-center gap-2 px-3 py-2 rounded-md bg-accent border border-border">
                    ...
                    <span className="flex-1 text-sm text-accent-foreground">
                      {form.locationName}
                    </span>
                    {!form.teamId && (
                      <button
                        ...
                        aria-label={t("content.discover.edit.removeLocation")}
                      >
                        <X className="h-4 w-4" />
                      </button>
                    )}
                  ...
                  <p className="rounded-md border border-border bg-accent px-3 py-2 text-sm text-muted-foreground">
                    {t("content.discover.edit.teamLocationLocked")}
                  </p>
                ) : (
              ...
              <FormField
                ...
                hint={t("content.discover.edit.tagsCountHint", {
          ...
          {/* Markdown input stays a small client island; preview is rendered separately. */}
        ...
        >
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              >
                {t("content.discover.edit.cancelTitle")}
              </h3>
              <p
                id="cancel-edit-desc"
                className="text-sm text-muted-foreground"
              >
                {t("content.discover.edit.cancelDesc")}
              </p>
            </div>
          </div>
