        ...
        <div className="grid grid-cols-1 lg:grid-cols-[3fr_2fr] gap-6">
          <div className="space-y-4">
            {[180, 220, 160].map((h, i) => (
              <div
                key={i}
                className="rounded-2xl bg-stone-200 dark:bg-stone-800 border border-stone-100 dark:border-stone-800"
                style={{ height: h }}
              />
            ))}
          </div>
...
function PreviewPanel({ data, regionName }: PreviewPanelProps) {
  ...
  return (
    <div className="sticky top-20">
      <div
        className="rounded-2xl bg-white dark:bg-stone-900 border border-stone-100 dark:border-stone-800 overflow-hidden"
      ...
      >
        <div className="flex items-center gap-2 px-4 py-3 border-b border-stone-50 dark:border-stone-800">
          ...
          <span className="text-xs font-semibold text-stone-600 dark:text-stone-400">
            {t("admin.previewEffect")}
          </span>
        </div>
        <div
          className="w-full bg-stone-100 dark:bg-stone-800"
          style={{ aspectRatio: "16/9" }}
        >
          {data.coverImageUrl ? (
            <img
              src={data.coverImageUrl}
              alt={t("admin.coverImagePreview")}
        ...
        <div className="p-4 space-y-3">
            ...
            <h3 className="font-bold text-stone-900 dark:text-stone-100 text-base leading-snug">
              {data.name || (
                <span className="text-stone-300 dark:text-stone-600">
                  {t("admin.locationNamePlaceholder")}
                </span>
              )}
            </h3>
            {data.subtitle && (
          ...
          {(regionName || data.address) && (
            <div className="flex items-start gap-1.5 text-xs text-stone-500 dark:text-stone-400">
              ...
              <span>
                {[regionName, data.address].filter(Boolean).join(" · ")}
              </span>
            </div>
          )}
          {data.description && (
...
function LocationFormClient({ locationId }: LocationFormClientProps) {
  ...
  const [savingIntent, setSavingIntent] =
    React.useState<LocationSaveIntent | null>(null);

  const currentRegionName = React.useMemo(
    () =>
      form.regions.find((region) => region.id === form.formData.regionId)
        ?.name ?? "",
  ...
  const progressSteps = [
    {
      id: "core",
      label: t("admin.progressStep1"),
      done: !!form.formData.name && !!form.formData.description,
    },
    {
      id: "location",
      label: t("admin.progressStep2"),
      done: !!form.formData.regionId,
    },
    {
      id: "media",
      label: t("admin.progressStep3"),
      done: !!form.formData.coverImageUrl,
    },
    {
      id: "finish",
      label: t("admin.progressStep4"),
      done:
        !!form.formData.name &&
        !!form.formData.description &&
        !!form.formData.regionId &&
        !!form.formData.coverImageUrl,
    },
  ];

  const submitLocation = React.useCallback(
    async (intent: LocationSaveIntent) => {
