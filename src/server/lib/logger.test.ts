...
describe("structured Worker logger", () => {
  ...
  it("accepts an explicit request ID without async-local state", async () => {
    const warn = vi.spyOn(console, "warn").mockImplementation(() => undefined);
    const first = "11111111-1111-4111-8111-111111111111";
    const second = "22222222-2222-4222-8222-222222222222";

    await Promise.all([
      Promise.resolve().then(() => logger.warn("first_async_warning", { requestId: first })),
      Promise.resolve().then(() => logger.warn("second_async_warning", { requestId: second })),
    ]);
