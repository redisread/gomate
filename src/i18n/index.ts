...
function getFromCache(ns: string, locale: Locale): Record<string, unknown> | null {
  ...
  // 直接从当前 HTML 文档的 SSR 数据读取。这个对象属于浏览器文档，
  // 不会写入 Worker isolate 的 module/global 状态。
...
export function hydrateSSRData(
  ...
  const ssrData =
    data ?? (window as unknown as Record<string, unknown>).__I18N_DATA__;
  if (!ssrData) return;
...
/**
 * 获取当前 locale。请求语言只来自 cookie/HTML，绝不写入 module/global
 * 状态；Worker isolate 可并发处理不同语言请求。
 */
export function getLocale(): Locale {
  // SSR 没有浏览器 cookie；Astro 页面用显式 locale 调用 getSSRT。
